...
first_seq = set(int(x) for x in input().split()) # first sequence
second_seq = set(int(x) for x in input().split()) # second sequence

n = int(input())

for _ in range(n):
    command, set_type, *nums = input().split()
    nums = [int(x) for x in nums] # convert str to int
